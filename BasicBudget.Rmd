...
raw.tree = read.csv("Data/live trees/live tree data & code/hf-c-trees-gmr-11may.csv",header=TRUE)
...
## subset trees to a common size threshold
## ultimately I'd like to drop this but I think it's a handy first pass
## to make data sets more comparable & computation more efficient
tapply(tree$dbh,tree$site,min,na.rm=TRUE)
#tree = tree[tree$dbh>=10,]
## Commented out because removal has to be done in WIDE format to accommodate ingrowth correctly

## Testing subset
tree = tree[which(tree$site %in% c("ems","hemlock","hemlock woodlot","red pine plantation","mixed hardwood","lyford","simes","TS","PH")),] 
#"pisgah" deopped because its in NH
...
### build plot census data for all trees
dup = c(14307,14037)
lyf.dbh  = spread(data = lyford.tree[-dup,c("tree","year","dbh")],year,dbh)
lyf.cond = spread(data = lyford.tree[-dup,c("tree","year","cond")],year,cond)
colnames(lyf.cond)[2:8] = paste0("COND",colnames(lyf.cond)[2:8])

##add COND to rings.plot.merge
#match(lyf.cond$tree,rings.plot.merge$tree)
rings.plot.merge = merge(rings.plot.merge,lyf.cond,by = "tree",all.x=TRUE)
