...
tree.mass <- exp(b0+b1*log(tree$dbh))
...
mup.median = matrix(NA,nsites,nyears)
for(y in 1:nyears){
  ci[y,] <- quantile(b.df[,paste0("mu[",y,"]")],c(0.025,0.5,0.975))
    for(s in 1:nsites){
    mup.median[s,y] = median(b.df[,paste0("mup[",s,",",y,"]")])
  }
}
...
for(s in 1:nsites){
  lines(years,mup.median[s,],lty=3,col=s,lwd=2)
...
AGBp.median = matrix(NA,nsites,nyears)
for(y in 1:nyears){
  agb = b.df[,paste0("mu[",y,"]")]*b.df[,paste0("rho[",y,"]")]*10 # kg/m2 -> Mg/ha
  ci.agb[y,] <- quantile(agb,c(0.025,0.5,0.975))
  for(s in 1:nsites){
    AGBp.median[s,y] = median(b.df[,paste0("rhop[",s,",",y,"]")]*b.df[,paste0("mup[",s,",",y,"]")])*10
  }
}
...
for(s in 1:nsites){
  lines(years,AGBp.median[s,],lty=3,col=s,lwd=2)
  points(years,agb.raw,col=s,lty=2,lwd=3)
}



## Plot the evolution of mean tree size and density
xlim=range(rhop.median)
ylim=range(mup.median)
yr=years-years[1]+1
plot(rhop.median[1,],mup.median[1,],type='n',xlim=xlim,ylim=ylim,
    xlab="Stem Density",ylab="Mean Tree Biomass")
for(i in 1:nsites){
  arrows(rhop.median[i,-nyears],mup.median[i,-nyears],rhop.median[i,-1],mup.median[i,-1],col=i,length=0.1)
}
```
