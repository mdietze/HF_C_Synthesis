...
dbh69.match <- as.data.frame(cbind(lyford.tree$tree[dbh69],lyford.tree$dbh[dbh69]))
colnames(dbh69.match) <- c("tree","dbh")

rings.plot.merge <- merge(rings.plot.merge,dbh69.match,by = "tree",all.x=TRUE)
colnames(rings.plot.merge)[colnames(rings.plot.merge) == "dbh"] <- "DBH69"

#1975 census
dbh75 <- which(lyford.tree$year == lyf.years[2])
dbh75.match <- as.data.frame(cbind(lyford.tree$tree[dbh75],lyford.tree$dbh[dbh75]))
colnames(dbh75.match) <- c("tree","dbh")
...
dbh91.match <- as.data.frame(cbind(lyford.tree$tree[dbh91],lyford.tree$dbh[dbh91]))
colnames(dbh91.match) <- c("tree","dbh")
...
dbh87.match <- as.data.frame(cbind(lyford.tree$tree[dbh87],lyford.tree$dbh[dbh87]))
colnames(dbh87.match) <- c("tree","dbh")
...
dbh92.match <- as.data.frame(cbind(lyford.tree$tree[dbh92],lyford.tree$dbh[dbh92]))
colnames(dbh92.match) <- c("tree","dbh")
...
dbh01.match <- as.data.frame(cbind(lyford.tree$tree[dbh01],lyford.tree$dbh[dbh01]))
colnames(dbh01.match) <- c("tree","dbh")
...
dbh11.match <- as.data.frame(cbind(lyford.tree$tree[dbh11],lyford.tree$dbh[dbh11]))
colnames(dbh11.match) <- c("tree","dbh")
...
allom.dir = "Allometry Papers & Analysis"
allom.files = dir(allom.dir,pattern = "*.Rdata")
allom.species = sub(pattern = "Allom.","",allom.files)
allom.species = sub(".3.Rdata","",allom.species)
allom.mcmc = list()
for(i in seq_along(allom.files)){
  load(file.path(allom.dir,allom.files[i])) ## load one allometry file
